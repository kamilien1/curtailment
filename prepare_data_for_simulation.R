...
# note: all data becomes hourly data generated over the course of a year
...
# -- annual, peak+min demand in gw, net demand in twh


# supply
# -- supplyForecast -- annual installed capacity, mw, by supply_type
# -- renewables
# --- wind -- hourly for one year wind by power factor (kwh/kW)
# --- solar
# ---- hourly, 1-day % of power generated by hour (sums to 1)
# ---- mnothly, 13-month capacity factor (cf * installed capacity = power produced)

## helper data
# --- chpDates -- start and end date of CHP plants
# --- chpRatios -- ratio of chp to non-chp plants
# --- supplyOutput -- pmin, pmax -- value of min and max amount of power available by power type
# coal and natgas are the two defaults


# transmission
# -- hourly, with season, 24-hr profile
# -- monthly, with season, % power flow relative to peak month
# -- annual, gw flow and ratio of power flow in versus out (1 = 100% in, 0 = 100% out)


# general
# -- hourly dates -- choose start and end year, make an hourly date range
...
setAnnualProfile <- function(df, hd, md, ad){
    ...
    temp <- left_join(temp, md, by = 'month','season' )
